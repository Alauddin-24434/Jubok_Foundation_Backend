...
async function bootstrap() {
  ...
  try {
    ...
    if (!projects || projects.length === 0) {
      logger.error('❌ No projects found. Please run "npm run seed" (for projects) first.');
      process.exit(1);
    }
    ...
    const bannersData = [
      {
        ...
        description: 'Join our community-driven organic farming projects and earn halal profits.',
      ...
      },
      {
        ...
        description: 'Direct investment in fish farming and poultry with complete transparency.',
        image: 'https://images.unsplash.com/photo-1524704054469-44931d7e234e?q=80&w=2848&auto=format&fit=crop',
        displayOrder: 2,
      },
      {
        title: 'Sustainable Real Estate',
        description: 'Secure your future with our exclusive Shariah-compliant housing projects.',
    ...
    for (let i = 0; i < bannersData.length; i++) {
        ...
        logger.log(`✅ Created banner: "${bannerInfo.title}" linked to "${project.name}"`);
    }
